...
public class SearchItemDecoration extends RecyclerView.ItemDecoration {
    ...
    private int mFirstAndLastColumnW = 2;          //您所需指定的间隔宽度，主要为第一列和最后一列与父控件的间隔；行间距，列间距将动态分配
     ...
     */
    public SearchItemDecoration(Context context, int firstAndLastColumnW) {
        mContext = context;
     ...
     */
    public SearchItemDecoration(Context context, int firstAndLastColumnW, int firstRowTopMargin, int lastRowBottomMargin) {
        mContext = context;
        mFirstAndLastColumnW = firstAndLastColumnW;
